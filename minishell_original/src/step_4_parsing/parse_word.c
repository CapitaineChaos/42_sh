...
bool	parse_word(t_tokens *tokens, t_deque *input)
{
	...
	token = tokens->head;
	if (token->kind != TKD_OPERAND)
		return (false);
	debug_nano_title(LVL_PARSER, "Parse words");
	if (input->head
	...
	while (token && token->kind == TKD_OPERAND)
	{
		...
		#pragma region "DEBUG"
		if (token->family == TKF_REDIRECT)
			trace_info_nvstr(LVL_PARSER, "Token redirect found    ", debug_get_token_type(token->type));
		else
			trace_info_nvstr(LVL_PARSER, "Token operand found     ", debug_get_token_type(token->type));
		#pragma endregion
		token = tokens->head;
	}
	return (true);
