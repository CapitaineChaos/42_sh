...
from .config import (LOCALE_CANDIDATES, SH42, SH42_SRC, PROJECT,
                     TEST_MAKE_ARGS)
from .ui import C
...
def build_binary(no_build: bool) -> bool:
    ...
    if no_build:
        if SH42.exists() and accepts_pipes(SH42):
            return False
        if SH42.exists():
    ...
    if not SH42_SRC.exists():
        sys.exit(f"{C.RED}Binaire introuvable : {SH42_SRC}{C.RESET}")
...
def preflight(base_env: dict) -> None:
    """Abandonne si 42_sh ne tourne pas en non-interactif (pipe in/out)."""
    checks: list[tuple[str, bool]] = []

    # pipe-in + capture stdout
    checks.append(("stdin pipe-in + stdout capturable", accepts_pipes(SH42)))

    # pipe-out : 42_sh | process
    ok_out = False
    try:
        p = subprocess.run(
            f"printf 'echo __pf__\\n' | {shlex.quote(str(SH42))} | cat",
            shell=True, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL,
            timeout=5, env=base_env)
        ok_out = b"__pf__" in p.stdout
    except Exception:
        ok_out = False
    checks.append(("pipe-out (42_sh | process)", ok_out))
    ...
    if not all(ok for _, ok in checks):
        sys.exit(f"{C.RED}Préflight échoué : 42_sh ne peut pas être testé "
