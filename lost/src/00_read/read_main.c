...
static void	manage_pendings(t_main_data *data)
{
		...
		|| data->lx->pending_squote)
	{
		...
		exstr_append_str(&data->total_input, "\n");
...
bool	main_loop(t_main_data *data)
{
	...
	exstr_append_str(&data->total_input, data->partial_input);
	data->lx->inp.stream = exstr_get(&data->total_input);
