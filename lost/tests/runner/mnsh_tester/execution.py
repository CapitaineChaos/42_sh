...
from .config import FIXTURES, MINISHELL, SIGNAL_ERRORS, VALGRIND_PATTERNS
...
def outfiles_brief_diff(a: Path, b: Path) -> list[str]:
    ...
    for name in sorted(set(sa) | set(sb)):
        if name not in sa:
            ...
            diffs.append(f"Only in minishell: {name}")
...
def run_one(case: Case, args, base_env: dict) -> Result:
    """Un cas : deux bacs à sable (minishell, bash), puis comparaison."""
    res = Result(index=case.index, cmd=case.label)
    with tempfile.TemporaryDirectory(prefix="mnsh_test_",
        ...
        mini_dir.mkdir()
        bash_dir.mkdir()
        build_sandbox(mini_dir)
        build_sandbox(bash_dir)

        vlog = (tmp / "valgrind.log") if args.valgrind else None
        mini_out, mini_err, res.mini_code = run_shell(
            [str(MINISHELL)], case.steps, mini_dir, base_env, args.timeout, vlog)
        bash_out, bash_err, res.bash_code = run_shell(
            ["bash", "--posix", "--noprofile", "--norc"],
            case.steps, bash_dir, base_env, args.timeout, None)

        res.mini_out = mini_out.rstrip("\n")
        res.bash_out = bash_out.rstrip("\n")
        res.mini_err = clean_stderr(mini_err)
        res.bash_err = clean_stderr(bash_err)
        res.outfiles_diff = outfiles_brief_diff(
            mini_dir / "outfiles", bash_dir / "outfiles")
        res.is_segfault = res.mini_code == 139
        if args.valgrind and res.mini_code != res.bash_code:
            res.critical = classify_valgrind(res.mini_code, vlog)

    res.ok_output = res.mini_out == res.bash_out
    res.ok_code = res.mini_code == res.bash_code
    res.ok_diff = not res.outfiles_diff
    res.ok_error = res.mini_err == res.bash_err
