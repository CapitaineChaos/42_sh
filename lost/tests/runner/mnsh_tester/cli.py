...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Testeur différentiel minishell/bash")
    cats = "/".join(categories()) or "aucune"
    p.add_argument("filters", nargs="*", help=f"catégorie ({cats}) ou fichier .tests/.toml")
    p.add_argument("-v", "--valgrind", action="store_true", help="exécuter minishell sous valgrind")
...
def main() -> int:
    ...
    print(f"{C.GREEN}🌴 Lapinou 🐰🐯 WiLDCaT 🌴 MiniShELL - "
          f"{args.jobs} workers · locale {locale}{C.RESET}\n")
    ...
    while True:
        ...
        set_title(f"tester {'✓' if ok else '✗'} {totals['strict']}/{totals['n']}")
